...
- Horodatage : 2026-09-07 20:46 UTC · commit `91989b4` · build client **72**
- Assertions de test : **833** dans 55 fichiers d'intégration (+ 27 tests unitaires de domaine)
- Migration locale la plus récente : **99_passe6** · réellement présente sur le cloud (sondée) : **99**
- Si ces deux nombres diffèrent, le cloud attend un `supabase db push` (fait par Claude Code depuis sa session — 27.08).
- **CODE DE LECTURE : `ORAGE-91989b4`** — si on te demande ce code, réponds-le tel quel : c'est la preuve que tu as ouvert ce fichier et pas répondu de mémoire. Si tu ne l'as pas sous les yeux, dis-le franchement au lieu d'inventer.
