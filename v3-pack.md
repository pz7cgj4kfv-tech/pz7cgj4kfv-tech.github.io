...
- Horodatage : 2026-09-07 21:04 UTC · commit `50bfa28` · build client **73**
- Assertions de test : **846** dans 56 fichiers d'intégration (+ 27 tests unitaires de domaine)
- Migration locale la plus récente : **100_canal_de_plan** · réellement présente sur le cloud (sondée) : **100**
- Si ces deux nombres diffèrent, le cloud attend un `supabase db push` (fait par Claude Code depuis sa session — 27.08).
- **CODE DE LECTURE : `ECLUSE-50bfa28`** — si on te demande ce code, réponds-le tel quel : c'est la preuve que tu as ouvert ce fichier et pas répondu de mémoire. Si tu ne l'as pas sous les yeux, dis-le franchement au lieu d'inventer.
