...
- Horodatage : 2026-09-06 10:37 UTC · commit `941546b` · build client **51**
- Assertions de test : **581** dans 43 fichiers d'intégration (+ 27 tests unitaires de domaine)
- Migration locale la plus récente : **74_moment_exclusif** · réellement présente sur le cloud (sondée) : **74**
- Si ces deux nombres diffèrent, le cloud attend un `supabase db push` (fait par Claude Code depuis sa session — 27.08).
- **CODE DE LECTURE : `ARDOISE-941546b`** — si on te demande ce code, réponds-le tel quel : c'est la preuve que tu as ouvert ce fichier et pas répondu de mémoire. Si tu ne l'as pas sous les yeux, dis-le franchement au lieu d'inventer.
